...
class RotorClass(object):
    ...
    def __init__(self, listname, backup, name):
    ...
    def defswitch(self, places):        #Switches and stays at that position
        for i in range(0, places):
            self.listname += [self.listname.pop(0)]
        return (self.listname)

    def reset(self):                    #Resets back to starting position
        self.listname = self.backup
        return(self.listname)

rotor_I = RotorClass(rotor_I_numbers, rotor_I_numbers_backup, "I")
