...
def crackenigma(message, guess):
    ...
    def Loop(currentlist, checker, turn, i):
        check = checker[:]
        comb = guess[turn] + i
        ...
        if not comb in currentlist and not comb[::-1] in currentlist and (comb[0] in checker or comb[1] in checker):
            ...
            comb = enigmaOne(guess[turn], rotorI, rotorII, rotorIII, reflectorB, turn, 0, 0, plugdict) + message[turn]
    ...
    for i in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
        checker = Loop(currentlist, checker, 0, i)
        if len(checker) > 0:
            if enigma(guess[:1], rotorI, rotorII, rotorIII, reflectorB, 0, 0, 0, DictionConvert(currentlist)) == message[:1]:

                for j in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
                    turn = 1
                    checker = Loop(currentlist, checker, turn, j)
                    if len(checker) > (turn * 4):
                        if enigma(guess[:(turn + 1)], rotorI, rotorII, rotorIII, reflectorB, 0, 0, 0, DictionConvert(currentlist)) == message[:(turn + 1)]:

                            for i in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
                                turn = 2
                                checker = Loop(currentlist, checker, turn, i)
                                if len(checker) > (turn * 4):
                                    if enigma(guess[:(turn + 1)], rotorI, rotorII, rotorIII, reflectorB, 0, 0, 0, DictionConvert(currentlist)) == message[:(turn + 1)]:
