                ...
                if code[2] in plugdiction: #WW
                    ...
                    if enigma(code[2], rotorI, rotorII, rotorIII, reflectorB, 2, 0, 0, plugdiction) in plugdiction:
                        ...
                        if code[3] in plugdiction: #WWW
                            #print("JA")
                            fourth_resultpair = enigma(code[3], rotorI, rotorII, rotorIII, reflectorB, 3, 0, 0, plugdiction) + origineel[3]
                            if enigma(code[3], rotorI, rotorII, rotorIII, reflectorB, 3, 0, 0, plugdiction) in plugdiction:
                                #("fout: wel_code[1] > wel_code[2]")
                                foutlijst.append(second_resultpair)
                                foutlijst.append(first_resultpair)
                                foutlijst.append(first_guess)
                                ...
                                if code[4] in plugdiction: #WWWW
                                    fifth_resultpair = enigma(code[4], rotorI, rotorII, rotorIII, reflectorB, 4, 0, 0, plugdiction) + origineel[4]
                                    if enigma(code[4], rotorI, rotorII, rotorIII, reflectorB, 4, 0, 0, plugdiction) in plugdiction:
                                        #("fout: wel_code[1] > wel_code[2]")
                                        foutlijst.append(second_resultpair)
                                        foutlijst.append(first_resultpair)
                                        foutlijst.append(first_guess)
                            ...
                            for x in alphabet:
                                ...
                                fourth_resultpair = enigma(fourth_guess[1], rotorI, rotorII, rotorIII, reflectorB, 2, 0, 0, plugdiction) + origineel[2]
                    ...
                    for x in alphabet:
                        ...
                        third_resultpair = enigma(third_guess[1], rotorI, rotorII, rotorIII, reflectorB, 1, 0, 0, plugdiction) + origineel[1]
                        ...
                        if code[3] in plugdiction: #WNW
                            ...
                            if enigma(code[3], rotorI, rotorII, rotorIII, reflectorB, 3, 0, 0, plugdiction) in plugdiction:
                                ...
                                if code[3] in plugdiction: #WNWW
                                    ...
                                    if enigma(code[4], rotorI, rotorII, rotorIII, reflectorB, 4, 0, 0, plugdiction) in plugdiction:
                                        ...
                                        if code[4] in plugdiction: #WNWWW
                                            sixth_resultpair = enigma(code[4], rotorI, rotorII, rotorIII, reflectorB, 4, 0, 0, plugdiction) + origineel[4]
                                            if enigma(code[4], rotorI, rotorII, rotorIII, reflectorB, 4, 0, 0, plugdiction) in plugdiction:
                                                #("fout: wel_code[1] > wel_code[2]")
                                                foutlijst.append(second_resultpair)
                                                foutlijst.append(first_resultpair)
                                    ...
                                    for x in alphabet:
                                        ...
                                        fifth_resultpair = enigma(fifth_guess[1], rotorI, rotorII, rotorIII, reflectorB, 3, 0, 0, plugdiction) + origineel[3]
                            ...
                            for x in alphabet:
                                ...
                                if code[3] in plugdiction: #WNNW
                                    fifth_resultpair = enigma(code[3], rotorI, rotorII, rotorIII, reflectorB, 3, 0, 0, plugdiction) + origineel[3]
                                    if enigma(code[3], rotorI, rotorII, rotorIII, reflectorB, 3, 0, 0, plugdiction) in plugdiction:
            ...
            for x in alphabet:
                ...
                if code[2] in plugdiction: #NW
                    ...
                    for x in alphabet:
                        ...
                        third_resultpair = enigma(third_guess[1], rotorI, rotorII, rotorIII, reflectorB, 3, 0, 0, plugdiction) + origineel[3]
                        first_list.append(third_guess)
                        first_list.append(third_resultpair)
...
print(cracking_enigma("MVAEDUUHBAVPD" ,"WETTERBERICHT"))
